<?php
	...
	//activity update
	$activity = mysql_real_escape_string("Viewing Private Forum on Mobile");
...
<?php startblock('content') ?>
	<div class="ui-grid-a">
		<div class="ui-block-a"><a href="/newthread.php" data-role="button" data-icon="plus" data-theme="b">New Thread</a></div>
		<div class="ui-block-b"><a href="/pubforum.php" data-role="button" data-icon="arrow-r" data-iconpos="right" data-theme="a">Public Forum</a></div>
	</div>
	<br />
	<ul id="priv-threads" data-role="listview">
	<?php
		$stickydivider = 1;
		while ($row = mysql_fetch_assoc($result)) {
			...
			if ($stickydivider == $sticky) {
				if ($stickydivider == 1) {
					echo "<li class='expandable' data-role=\"list-divider\"><span class='ui-icon ui-icon-plus ui-icon-shadow icon-nobutton'>&nbsp;</span> Sticky Threads</li>";
					$stickydivider = 0;
				} else {
	...
	<br />
	<input type="hidden" id="priv-start" value="<?php echo $start; ?>" />
	<input type="hidden" id="priv-count" value="<?php echo $count; ?>" />
	<input type="hidden" id="priv-total" value="<?php echo $totalthreads; ?>" />
	<button id="load-priv-threads" data-href="/ajax/loadprivthreads.php" data-icon="refresh" data-theme="a" data-mini="true">Load More</button>
<?php endblock() ?>